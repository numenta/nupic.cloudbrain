...
from cloudbrain.analyzer.htm.algorithms.sequence_classifier_factory import SequenceClassifierFactory
...
class SequenceClassifierRegion(PyRegion):
  ...
  def getSpec(cls):
    ...
    spec = {
        ...
        # TODO: compute() needs to be implemented
        ...
        'parameters': {
          ...
          #TODO: this should go away for sequence classification. we always ma TM states to current timestep
          'steps': {
          ...
          'implementation': {
            ...
            'dataType': 'Byte',
            'count': 0,
            'constraints': 'enum: py' # TODO: Removed 'cpp' impl, since it doesn't exist. Check if it is ok.
          },
  ...
  def compute(self, inputs, outputs):
    ...
    self.recordNum += 1
    self.classificationResult = clResults["actualValues"][clResults[int(self.steps)].argmax()]
